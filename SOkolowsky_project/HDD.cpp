...
//FAT TABLE - separate table, where info about files are keeping (like catalog) In oryginal disk FAT TABLE is integrated with, but this disk is to small to do it
	...
	bool HDD::check_file_exist(std::string file_to_save_name) {
		for (int i = 0; i < FAT_table.size(); i++) {//iterting through the FAT TABLE
			if (FAT_table[i].file_name == file_to_save_name) {//comparisoning file's name
	...
	bool HDD::check_space(int file_to_save_size) {
		...
		for (int j = 0; j < block_number; j++) {//calculating free space on disk
			if (data_conteiner[j] == '1') { //checking if in BITS VECTOR in position 'j' is free space (if is, program finds one free block on disk)
	...
	int HDD::check_first_empty_block() {
		for (int i = 0; i < block_number; i++) { //checking first empty block for data's index in bit's vector
			if (data_conteiner[i] == '1') { //when sector is empty and we can save index in this position the value of cell is set to '1'
				data_conteiner[i] = '0'; //setting value to '0'
	...
	void HDD::uniforming_format_of_data_index(int index_index_block, int index_data_block, int counter_tmp) {
		if (index_data_block < 10) {
			data_conteiner[(index_index_block*block_size) + counter_tmp] = '0';
			data_conteiner[((index_index_block*block_size) + 1) + counter_tmp] = index_data_block + '0';
		}//end of if
		else {
			data_conteiner[(index_index_block*block_size) + counter_tmp] = (index_data_block / 10) + '0';
			data_conteiner[((index_index_block*block_size) + 1) + counter_tmp] = (index_data_block % 10) + '0';
		}//end of else
	}
	//EXTENSIONS
	void HDD::detailed_disk_view() {
		...
		for (int i = 0; i < data_conteiner.size(); i++) {
			std::cout << "[" << data_conteiner[i] << "]";
			if ((i + 1) % block_size == 0) {
				std::cout << "\n";
			}//end of if
		}//end of for
	...
	void HDD::fat_table_view() {
		...
		if (FAT_table.size() == 0) {
			...
			for (int i = 0; i < FAT_table.size(); i++) {
				std::cout << FAT_table[i].file_name << "\t" << FAT_table[i].file_size << "\t" << FAT_table[i].first_block_index << "\n";
			}//end of for
		}
	}
	...
	HDD::HDD() {
		for (int i = 0; i < block_number; i++) { // loop which iterate through the conteiner
			data_conteiner[i] = '1'; //filling conteiner with ones
		}//end of for
		data_conteiner[0] = '0'; //first blick is always (there is a bit vector here)
	}
	HDD::~HDD() {
	}
	void HDD::create_file(std::string file_to_save_name, int file_to_save_size, std::string file_to_save_text) {
		if (check_space(file_to_save_size) == true) {//checking free space
			if (check_file_exist(file_to_save_name) == false) { //checking free name
				...
				FAT_table.push_back(file_to_save); // saving new file to FAT table

				//uprawnienia.createACL(file_to_save_name);
				...
				while (residual_size > 0) { //saving data when some bits left
					...
					for (int i = 0; i < block_size; i++) { //filling sector
						if ((file_to_save_text.size() - residual_size) >= 0 && (file_to_save_text.size() - residual_size) != file_to_save_text.size()) { //checking range
							data_conteiner[(block_size * index_data_block) + i] = file_to_save_text[file_to_save_text.size() - residual_size];//filling bits in block
	...
	void HDD::rename_file(std::string file_old_name, std::string file_new_name) {
		if (check_file_exist(file_old_name)) {//checking existing of file
			for (int i = 0; i < FAT_table.size(); i++) {//searching index of file in FAT
				if (FAT_table[i].file_name == file_old_name) {//looging for file's old name
					FAT_table[i].file_name = file_new_name;//changing name
	...
	void HDD::format_disk() {
		...
		for (int i = block_size; i < disc_size; i++) {
			data_conteiner[i] = NULL;
		}

		FAT_table.erase(FAT_table.begin(), FAT_table.end());
	...
	std::string HDD::read_file(std::string file_to_read_name) {
		...
		if (check_file_exist(file_to_read_name) == true) {
			for (int i = 0; i < FAT_table.size(); i++) {
				if (FAT_table[i].file_name == file_to_read_name) {
					index_index_block = FAT_table[i].first_block_index;
					residual_size = FAT_table[i].file_size;
					break;
				}//if
			...
			while (i<block_size - 1 && residual_size>0) {//REFAKTORYZACJA!
				...
				if (i == (block_size - 2) && residual_size > block_size) {
					actual_index_to_read = ((data_conteiner[(index_index_block * 32) + i] - '0') * 10) + (data_conteiner[(index_index_block * 32) + i + 1] - '0');
					index_index_block = actual_index_to_read;
					i = 0;
				}

				actual_index_to_read = (data_conteiner[index_index_block*block_size + i] - '0') * 10 + (data_conteiner[index_index_block*block_size + i + 1] - '0');
				for (int j = 0; j < block_size; j++) {
					if (residual_size <= 0 || data_conteiner[actual_index_to_read*block_size + j] == NULL) {
						break;
					}
					file_content = file_content + data_conteiner[actual_index_to_read*block_size + j];
					residual_size--;
				}
				i = i + 2;
			}
	...
	void HDD::delete_file(std::string file_to_delete_name) {
		...
		if (check_file_exist(file_to_delete_name) == true) {
			for (int i = 0; i < FAT_table.size(); i++) {
				if (FAT_table[i].file_name == file_to_delete_name) {
					index_index_block = FAT_table[i].first_block_index;
					residual_size = FAT_table[i].file_size;
					data_conteiner[index_index_block] = '1';
					std::swap(FAT_table[i], FAT_table[FAT_table.size() - 1]);
					FAT_table.erase(FAT_table.begin() + (FAT_table.size() - 1));
					break;
				}//if
			...
			while (i<block_size - 1 && residual_size>0) {//REFAKTORYZACJA!
				...
				if (i == (block_size - 2) && residual_size > block_size) {
					actual_index_to_delete = ((data_conteiner[(index_index_block * 32) + i] - '0') * 10) + (data_conteiner[(index_index_block * 32) + i + 1] - '0');
					data_conteiner[actual_index_to_delete] = '1';
					index_index_block = actual_index_to_delete;
					i = 0;
				}

				//std::cout << data_conteiner[index_index_block*block_size + i] << "\n";//TEST
				actual_index_to_delete = (data_conteiner[index_index_block*block_size + i] - '0') * 10 + (data_conteiner[index_index_block*block_size + i + 1] - '0');
				data_conteiner[actual_index_to_delete] = '1';
